...
`src/common/evaluation-scoring.ts` is the evaluation rubric: nine criteria scored 1–5 in three weighted categories (Work Performance 40%, Professional Behavior 30%, Technical Skills 30%), and the bands that turn an average into a performance level. `overallRating` and `performanceLevel` are computed there and **stored** on the row — never accepted from the request, and `forbidNonWhitelisted` rejects a body that tries. The per-category `categories` breakdown is recomputed on read instead. The supervisor writes evaluations and the coordinator reads them across all establishments, so both go through this one module.
...
Prisma schema notes: `Attendance` splits AM/PM into four nullable `DateTime`s (`timeInAM`/`timeOutAM`/`timeInPM`/`timeOutPM`) and carries `@@unique([studentId, date])` (added by `20260811085904_attendance_one_per_day`) — `submitAttendance` normalises `date` to UTC midnight on write, so one student gets at most one row per calendar day and a repeat submission is a `ConflictException` (409). `Document`, `Credential`, `Conversation`/`ConversationParticipant`/`Message` are modeled and migrated but have no module yet; `socket.io` and `@nestjs/websockets` are installed with no gateway written. Deleting an establishment is blocked in `EstablishmentService.remove` when students or supervisors reference it, and deleting a student is blocked in `CoordinatorService.removeStudent` when attendance/evaluations/documents reference them — both throw `ConflictException` (409).
...
- `app/<role>/<page>/page.tsx` — thin, `"use client"`, composes a `Sidebar` + feature components. Each role's nav lives in `features/<role>/nav.ts` (`COORDINATOR_NAV`, `SUPERVISOR_NAV`, `STUDENT_NAV`) — import it, don't redeclare it inline.
...
**How much actually exists:** eleven real pages — `login`, `coordinator/{dashboard,establishments,students,evaluations,attendance}`, `student/{dashboard,attendance}`, `supervisor/{dashboard,attendance,evaluation}` — and eight API slices: `authApi`, `establishmentApi`, `studentApi` (coordinator's view), `studentPortalApi` (student's own view), `supervisorApi`, `evaluationApi`, `dashboardApi`, `attendanceOversightApi`. **All three roles now land on a real page after login, and `coordinator/dashboard` is backed by real aggregates** (`GET /coordinator/dashboard`) rather than mock constants. Still empty: `app/coordinator/messages`, `app/student/*` beyond the two above, `app/supervisor/messages`, and `types/`. `establishment`, `student` and `supervisor` are the end-to-end vertical slices; copy their shape when building a new domain.

A field with no backend (unread messages, pending documents) is **absent from the `dashboard` response, not `0`** — that's the pattern for any future aggregate too: don't report a real-looking zero for a feature that doesn't exist yet.
