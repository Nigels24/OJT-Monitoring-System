...
**How much actually exists:** ten real pages — `login`, `coordinator/{dashboard,establishments,students,evaluations}`, `student/{dashboard,attendance}`, `supervisor/{dashboard,attendance,evaluation}` — and six API slices: `authApi`, `establishmentApi`, `studentApi` (coordinator's view), `studentPortalApi` (student's own view), `supervisorApi`, `evaluationApi`. **All three roles now land on a real page after login.** Still empty: `app/coordinator/{attendance,messages}`, `app/student/*` beyond the two above, `app/supervisor/messages`, and `types/`. `coordinator/dashboard` renders module-level mock constants (`ATTENDANCE_TREND`, `TOP_ESTABLISHMENTS`, `RECENT_STUDENTS`) and hardcoded `stats` set in a `useEffect` with a `TODO` — there is no dashboard-stats endpoint. `establishment`, `student` and `supervisor` are the end-to-end vertical slices; copy their shape when building a new domain.
