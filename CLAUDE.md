...
**How much actually exists:** ten real pages — `login`, `coordinator/{dashboard,establishments,students,evaluations}`, `student/{dashboard,attendance}`, `supervisor/{dashboard,attendance,evaluation}` — and seven API slices: `authApi`, `establishmentApi`, `studentApi` (coordinator's view), `studentPortalApi` (student's own view), `supervisorApi`, `evaluationApi`, `dashboardApi`. **All three roles now land on a real page after login, and `coordinator/dashboard` is backed by real aggregates** (`GET /coordinator/dashboard`) rather than mock constants. Still empty: `app/coordinator/{attendance,messages}`, `app/student/*` beyond the two above, `app/supervisor/messages`, and `types/`. `establishment`, `student` and `supervisor` are the end-to-end vertical slices; copy their shape when building a new domain.
